...
#ifdef USE_MKL
    /// @brief Converts the Triplet (aka COO) matrix to CSR matrix (using MKL functions)
    /// @return Three vectors: row_pointers, column_indices, and values
    /// @note (TODO: Intel to confirm this) If there are duplicates, the resulting column_indices and values arrays will have larger sizes. Thus, one must check the resulting nnz value.
    sparse_matrix_t to_csr_using_mkl();

    /// @brief Prints the CSR matrix generated by Intel MKL
    /// @param csr The intel handle to the CSR matrix
    void print_intel_csr_matrix(sparse_matrix_t &csr, INT dimension);
#endif
};
